...
## 개요
- `store_dcinside_posts.py`, `store_reddit_posts.py`가 `item_id`를 RabbitMQ 큐에 발행하면 `dcinside_worker.py`, `reddit_worker.py`가 이를 소비해 상세 수집과 Gemini 요약을 수행한다.
- Cloud Run 등 HTTP 기반 환경에서 동작하도록 설계되어 있으며, 요청 1회당 큐에서 최대 1건을 소비한다.

## 실행 흐름
1. 수집 스크립트가 큐에 `{ "item_id": 123 }` 형태의 JSON 메시지를 발행한다.
2. 외부 트리거가 워커 HTTP 엔드포인트에 `POST /` 요청을 보낸다.
3. 워커는 큐에서 메시지를 하나 꺼내 DB에서 해당 아이템을 조회한다.
4. 상세 페이지/Reddit API를 호출해 본문·댓글·이미지를 확보하고, Gemini 요약을 생성한다.
...
- Gemini 설정 (`GEMINI_API_KEY`, `GEMINI_MODEL_PRIORITIES`, `GEMINI_TIMEOUT`, `GEMINI_MAX_TEXT`, `GEMINI_DEBUG`, `GEMINI_MODEL_COOLDOWN`).
