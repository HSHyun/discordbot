# 배경
현재 게시물을 크롤링 후 Gemini(기존 Codex)로 요약문을 생성한 뒤 저장하는건 해당 게시물 열에 저장하게 됨
...
# 요구사항
1. 크롤링 파이프라인은 기존과 동일하게 게시물/댓글을 수집하되, Gemini 등으로 생성한 요약문을 새로운 테이블에 분리 저장한다.
2. 새로운 테이블은 `item_summary`로 하고, 스키마/제약은 아래와 같이 정의한다.
	...
	- `model_name` TEXT NOT NULL — 사용한 요약 모델명(예: `gemini-2.5-flash`, `gmini`)
