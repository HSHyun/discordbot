...
## 역할
- DCInside 게시물 한 건을 선택해 Gemini 요약 파이프라인을 빠르게 검증하는 테스트 스크립트.
- 기존 본문/이미지 다운로드·요약 함수를 그대로 호출해 실제 파이프라인과 동일한 방식으로 동작 여부를 확인한다.

## 핵심 흐름
1. `.env`를 읽어 Gemini와 DB 설정을 초기화한다.
...
4. 이미지는 임시 디렉터리(`data/test-assets/dcinside`)에 다운로드하고, Gemini 요약이 끝나면 기본적으로 삭제한다(`--keep-assets`로 유지 가능).
5. Gemini 요약 결과와 입력 텍스트·이미지 목록, 참고한 댓글을 콘솔에 출력한다.
...
## 운영 시 유의 사항
- Gemini API 키와 `.env` 설정이 사전에 완료돼 있어야 한다.
